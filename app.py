...
@st.cache_data
def load_data():
    df = pd.read_csv("cric_info.csv", low_memory=False)
    ...
    df["runs_on_ball"] = (df["runs_batter"] + df["extras_wides"] + df["extras_noballs"] + df["extras_byes"] + df["extras_legbyes"])
    def get_phase(over):
...
ax1.legend()
st.pyplot(fig1)
st.info("Winning the toss only increases win probability by ~7–8%.")
...
ax2.bar(x - 0.2, phase_summary.loc[phase_order, "Won"].values,  width=0.4, color="green", label="Winner")
ax2.bar(x + 0.2, phase_summary.loc[phase_order, "Lost"].values, width=0.4, color="red",   label="Loser")
ax2.set_xticks(x)
...
ax2.legend()
st.pyplot(fig2)
st.info("Winning teams dominate all 3 phases but the gap is biggest in Death Overs.")

# Q3
st.header("Q3 — Top 5 Batters & Bowlers")
top_batters = df.groupby("batter")["runs_batter"].sum().reset_index().rename(columns={"runs_batter":"Total Runs","batter":"Batter"}).sort_values("Total Runs",ascending=False).head(5).reset_index(drop=True)
top_batters.index += 1
legal_df = df[(df["extras_wides"]==0) & (df["extras_noballs"]==0)]
bowler_wickets = df[df["wicket_kind"].notna() & (df["wicket_kind"]!="run out")].groupby("bowler")["wicket_kind"].count().reset_index().rename(columns={"wicket_kind":"Total Wickets"})
...
top_bowlers = bowlers_df.sort_values("Total Wickets",ascending=False).head(5).reset_index(drop=True)
top_bowlers.index += 1
...
st.info("These batters score far more in death overs than powerplay — pure finishers who thrive under pressure.")
